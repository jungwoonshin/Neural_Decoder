...
def split_edges(adj, dataset):
    ...
    train_index_i = indexes_train[0][:adj_train.shape[0]+len(train_edges)*2]
    train_index_j = np.array(indexes_train[1][:adj_train.shape[0]+len(train_edges)*2]) + len(u2id)
    ...
    for x,y in train_index[:len(train_edges)*2]:
    ...
    # train_edges = 
    split_edge_list = [ train_edges, train_false_edges, val_edges, val_edges_false, test_edges, test_edges_false ]
    split_edge_name = ['train_pos_edge_index','train_neg_edge_index','val_pos_edge_index','val_neg_edge_index','test_pos_edge_index','test_neg_edge_index']
    
    split_edge = types.SimpleNamespace()
    for index, edge in enumerate(split_edge_list):
        edge = np.transpose(np.array(edge))
        edge = torch.from_numpy(edge).long()
        name = split_edge_name[index]
        print(name,edge.shape)
        setattr(split_edge, name, edge)

    train_edges = split_edge.train_pos_edge_index
    train_edges_reverse = torch.cat([train_edges[1,:].view(1,-1), train_edges[0,:].view(1,-1)],dim=0)
    train_edges = torch.cat([train_edges, train_edges_reverse],dim=1)

    split_edge.train_pos_edge_index = train_edges
    # print(split_edge)
    return split_edge
